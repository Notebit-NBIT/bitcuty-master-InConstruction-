...
Node::P2PProtocolBitcuty::P2PProtocolBitcuty(Node *node, P2PClient *client)
    : P2PProtocolBasic(node->m_config, node->m_p2p.get_unique_number(), client)
    , m_node(node)
    , m_chain_timer(std::bind(&P2PProtocolBitcuty::on_chain_timer, this))
    , m_download_timer(std::bind(&P2PProtocolBitcuty::on_download_timer, this))
    , m_syncpool_timer(std::bind(&P2PProtocolBitcuty::on_syncpool_timer, this))
    , m_download_transactions_timer(std::bind(&P2PProtocolBitcuty::on_download_transactions_timer, this)) {}

Node::P2PProtocolBitcuty::~P2PProtocolBitcuty() = default;

CoreSyncData Node::P2PProtocolBitcuty::get_my_sync_data() const {
...
std::vector<NetworkAddress> Node::P2PProtocolBitcuty::get_peers_to_share() const {
	return m_node->m_peer_db->get_peerlist_to_p2p(
	    get_address(), m_node->m_p2p.get_local_time(), config.p2p_default_peers_in_handshake);
}

std::vector<PeerlistEntryLegacy> Node::P2PProtocolBitcuty::get_legacy_peers_to_share() const {
	return m_node->m_peer_db->get_peerlist_to_p2p_legacy(
	    get_address(), m_node->m_p2p.get_local_time(), config.p2p_default_peers_in_handshake);
}

void Node::P2PProtocolBitcuty::on_first_message_after_handshake() {
...
void Node::P2PProtocolBitcuty::advance_blocks() {
...
void Node::P2PProtocolBitcuty::advance_transactions() {
...
bool Node::P2PProtocolBitcuty::on_transaction_descs(const std::vector<TransactionDesc> &descs) {
...
void Node::P2PProtocolBitcuty::transaction_download_finished(const Hash &tid, bool success) {
...
bool Node::P2PProtocolBitcuty::on_idle(std::chrono::steady_clock::time_point idle_start) {
...
void Node::P2PProtocolBitcuty::after_handshake() {
...
void Node::P2PProtocolBitcuty::on_msg_handshake(p2p::Handshake::Request &&req) {
	m_node->m_peer_db->add_incoming_peer(get_address(), m_node->m_p2p.get_local_time());
	after_handshake();
}

void Node::P2PProtocolBitcuty::on_msg_handshake(p2p::Handshake::Response &&req) {
...
void Node::P2PProtocolBitcuty::on_msg_notify_request_chain(p2p::GetChain::Request &&req) {
...
void Node::P2PProtocolBitcuty::on_msg_notify_request_chain(p2p::GetChain::Response &&req) {
...
void Node::P2PProtocolBitcuty::on_msg_notify_request_objects(p2p::GetObjects::Request &&req) {
...
void Node::P2PProtocolBitcuty::on_msg_notify_request_objects(p2p::GetObjects::Response &&req) {
...
void Node::P2PProtocolBitcuty::on_disconnect(const std::string &ban_reason) {
...
void Node::P2PProtocolBitcuty::on_msg_notify_request_tx_pool(p2p::SyncPool::Request &&req) {
	if (req.from <= req.to)
...
void Node::P2PProtocolBitcuty::on_msg_notify_request_tx_pool(p2p::SyncPool::Response &&req) {
	m_syncpool_timer.cancel();
	m_syncpool_request_sent = false;
...
void Node::P2PProtocolBitcuty::on_msg_timed_sync(p2p::TimedSync::Notify &&req) { advance_chain(); }

void Node::P2PProtocolBitcuty::on_msg_notify_new_block(p2p::RelayBlock::Notify &&req) {
...
void Node::P2PProtocolBitcuty::on_msg_notify_new_transactions(p2p::RelayTransactions::Notify &&req) {
...
void Node::P2PProtocolBitcuty::on_msg_notify_checkpoint(p2p::Checkpoint::Notify &&req) {
...
#if bitcuty_ALLOW_DEBUG_COMMANDS

void Node::P2PProtocolBitcuty::on_msg_stat_info(p2p::GetStatInfo::Request &&req) {

...
static const char USAGE[] = R"(bitcutyd )" bitcuty_VERSION_STRING R"(.
...
Options:
  ...
  --bitcutyd-bind-address=<ip:port>     IP and port for bitcutyd RPC API [default: 127.0.0.1:8081].
  ...
  --data-folder=<folder-path>            Folder for blockchain, logs and peer DB [default: %appdata%/bitcuty].
  ...
  --bitcutyd-authorization=<usr:pass>   HTTP basic authentication credentials for RPC API.
  --bitcutyd-authorization-private=<usr:pass>   HTTP basic authentication credentials for get_statistics and get_archive methods.
...
int main(int argc, const char *argv[]) try {
	...
	if (const char *pa = cmd.get("--backup-blockchain")) {
		...
		if (cmd.show_errors("cannot be used with --backup-blockchain"))
			return api::BITCUTYD_WRONG_ARGS;
	...
	std::cout << "bitcutyd started seconds=" << double(idea_ms.count()) / 1000 << std::endl;
	while (!io.stopped()) {
